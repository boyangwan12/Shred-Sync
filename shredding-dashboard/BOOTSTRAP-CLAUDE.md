...
| "What's tomorrow's workout?" or "pre-log workout" | Read tomorrow's day_type from cycle. If rest → none. Else pull last session of same day_type, apply progression rules (RPE ≤8 → +1 rep; RPE 9-10 → match; missed reps → drop 5%), pull baselines from `claude-memory/project_strength_baselines.md`, write `workoutExercise` + `workoutSet` rows, write 1-paragraph briefing to `daily_log.notes`. This is the workout-side equivalent of `mealPlan` for food. |
