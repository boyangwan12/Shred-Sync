# Memory Index

- [User fitness goals](user_fitness_goals.md) — bodybuilder, 9-week cut 14.3%→10% BF, wants data-driven decisions
- [Notion hub](reference_notion_hub.md) — "Shredding Phase — Apr 7 to Jun 30" is the source of truth for all ShredSync data
- [Turso credentials](reference_turso_creds.md) — DB URL + access patterns; auth token lives in .env (gitignored), never commit it
- [Daily data prompt](feedback_daily_data_prompt.md) — on daily check-ins, proactively ask for weight / morning HR / energy / satiety / sleep-sync before analysis
- [Protocol design preferences](feedback_protocol_design.md) — durable design rules for the daily coaching protocol set during the 2026-04-27 deep interview (logistics-driven schedule, no duplicate analytical work, single 9 PM food touchpoint, falsifiable predictions, 3 interaction modes, workout pre-log via 9:30 PM cron)
- [Calibrated targets](project_calibrated_targets.md) — data-derived calorie/macro targets per day type, replacing Notion formulas (real maintenance ~2,234 kcal)
- [Strength baselines](project_strength_baselines.md) — per-exercise working weights as of Apr 25; shoulder press is 一边50 not 一边45, lateral raise is 15 not 10
